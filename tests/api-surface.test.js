...
class FakeSubscriptionManager {
  ...
  subscribe(subscription, policy, errorHandler, deltaHandler) {
    ...
    return () => {
      const idx = this.subscriptions.findIndex((s) => s.subscription === subscription);
      if (idx >= 0) this.subscriptions.splice(idx, 1);
    };
  }
}
...
test.describe("API Surface Tests", () => {
  test("matrix module exports required functions", () => {
    ...
    assert.strictEqual(typeof matrixModule.loadAllMatrices, "function",
      "matrixModule.loadAllMatrices must be a function (used by deps.loadMatrices)");
    assert.strictEqual(typeof matrixModule.saveMatrices, "function",
      "matrixModule.saveMatrices must be a function (used by deps.saveMatrices)");
    assert.strictEqual(typeof matrixModule.loadMatrix, "function",
      "matrixModule.loadMatrix must be a function");
    assert.strictEqual(typeof matrixModule.saveMatrix, "function",
      "matrixModule.saveMatrix must be a function");
  });

  test("schema module exports validateConfig", () => {
    assert.strictEqual(typeof schemaModule.validateConfig, "function",
  ...
  test("schema validateConfig returns expected structure", () => {
    ...
    assert.strictEqual(typeof result.valid, "boolean", "result.valid is a boolean");
    ...
    assert.strictEqual(result.warnings.length, 0, "Empty config has no warnings");
  });
  ...
  test("schema validateConfig detects duplicate paths", () => {
    ...
    assert.ok(result.warnings.some(w => w.includes("Duplicate power path")),
  ...
  test("fake app implements required Signal K API", () => {
    ...
    assert.strictEqual(typeof app.getSelfPath, "function",
      "app.getSelfPath must be a function");
    assert.strictEqual(typeof app.setSelfPath, "function",
      "app.setSelfPath must be a function");
    assert.strictEqual(typeof app.getDataPath, "function",
      "app.getDataPath must be a function");
    assert.strictEqual(typeof app.debug, "function",
      "app.debug must be a function");
    assert.strictEqual(typeof app.info, "function",
      "app.info must be a function");
    assert.strictEqual(typeof app.warn, "function",
      "app.warn must be a function");
    assert.strictEqual(typeof app.error, "function",
      "app.error must be a function");
    assert.strictEqual(typeof app.setPluginStatus, "function",
      "app.setPluginStatus must be a function");
    assert.strictEqual(typeof app.subscriptionmanager, "object",
      "app.subscriptionmanager must be an object");
    assert.strictEqual(typeof app.subscriptionmanager.subscribe, "function",
      "app.subscriptionmanager.subscribe must be a function");
  });

  test("subscriptionmanager.subscribe returns unsubscribe function", () => {
    const app = new FakeSignalKApp();
    const subscription = { context: "vessels.self", subscribe: [{ path: "test.path" }] };

    const unsubscribe = app.subscriptionmanager.subscribe(subscription, [], () => {}, () => {});
