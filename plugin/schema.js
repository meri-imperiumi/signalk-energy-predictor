...
function buildPluginSchema() {
  return {
    ...
    properties: {
      ...
      solarArrays: {
        ...
        items: {
          ...
          properties: {
            ...
            reluctance: {
              type: "string",
              title: "Reluctance to deploy",
              description:
                "How willing the crew is to deploy/stow this source. Low: deploy even for an hour of output. Medium: deploy when a couple hours of output are available. High: deploy only when you can get most of a day of output. Leave blank to use the type default (FLINsail arrays are typically Medium).",
              enum: ["low", "medium", "high"],
              enumNames: {
                low: "Low — deploy even for an hour of output",
        ...
        items: {
          ...
          properties: {
            ...
            reluctance: {
              ...
              title: "Reluctance to deploy",
              description:
                "How willing the crew is to deploy/stow this generator. Low: deploy even for an hour of output. Medium: deploy when a couple hours of output are available. High: deploy only when you can get most of a day of output. Leave blank to use the type default (hydro generators are typically Low; wind generators typically High).",
              enum: ["low", "medium", "high"],
              enumNames: {
                low: "Low — deploy even for an hour of output",
                medium: "Medium — deploy for a couple hours of output",
      ...
      weather: {
        ...
        properties: {
          ...
          forecastCacheHours: {
            ...
            description:
              "How long a tier-1/tier-2 forecast stays usable in memory before the FSM re-attempts a fetch. Cached forecast hours that are still in the future are always served while offline, regardless of this window. Offshore with Internet once a day, keep this at 24h.",
            default: 24,
            minimum: 1,
            maximum: 168,
          },
      ...
      surplus: {
        type: "object",
        title: "Surplus Energy Alerting",
        description:
          "Notify when the battery is forecast full while yield continues, so the charge controller would curtail energy that could run opportunistic loads (watermaker, ice maker). Includes the motoring side-effect case (arriving by motor at midday with hours of sun left)",
        properties: {
          enabled: {
            type: "boolean",
            title: "Enable Surplus Alerting",
            description:
              "Publish a surplus-energy opportunity notification and Signal K value when the battery is forecast full with yield to spare",
            default: true,
          },
          fullThreshold: {
            type: "number",
            ...
            description:
              "SoC at which the bank is considered full (absorption/full), 0-1",
            default: 0.95,
            minimum: 0.8,
            maximum: 1,
          },
          minSurplusWh: {
            type: "number",
            title: "Minimum Surplus (Wh)",
            description:
              "Minimum wasted-potential energy required to trigger an alert. Below this the surplus is too small to act on",
            default: 300,
            minimum: 0,
          },
          maxLeadHours: {
            type: "number",
            title: "Max Lead Time (hours)",
            description:
              "Only alert for surplus windows starting within this many hours. Beyond that, forecast uncertainty makes the alert noise",
            default: 36,
            minimum: 24,
            maximum: 48,
          },
          opportunisticLoads: {
            type: "array",
            title: "Opportunistic Loads",
            description:
              "Loads that can usefully consume surplus energy. Shown as suggestions in the alert message. Advisory-only — no load switching",
            items: {
              type: "object",
              properties: {
                name: {
                  type: "string",
                  title: "Name",
                  description: "e.g. Watermaker, Ice maker, Starlink",
                  default: "Watermaker",
                },
                watts: {
                  type: "number",
                  title: "Typical Power (W)",
                  description:
                    "Typical draw when running, used to estimate how long the surplus could run it",
                  default: 150,
                  minimum: 1,
                },
                statePath: {
                  type: "string",
                  title: "State Path (optional)",
                  description:
                    "Signal K path indicating whether this load is already running. A boolean true (digital switching) or a string matching onValues means running; the load is then not suggested as a surplus use since it is already consuming power. Leave blank for loads that aren't instrumented",
                  default: "",
                },
                onValues: {
                  type: "string",
                  title: "On Values (optional)",
                  description:
                    "Comma-separated values at statePath that mean the load is running, e.g. 'online,started,on'. Defaults to 'started,on,online,running,active'",
                  default: "started,on,online,running,active",
                },
              },
              required: ["name", "watts"],
            },
      ...
      notification: {
        type: "object",
        title: "Notification Urgency",
        description:
          "Multi-level notification urgency. Maps deploy/stow and battery advisories to Signal K notification states (alert/warn/alarm) and methods (visual/sound) based on intensity, time-to-action, event duration, and how confident we are about the detected deployment state. The urgency ladder (info→low→medium→high) is designed to prevent ever reaching alarm, which is reserved for vessel-safety-compromised situations (e.g. battery about to cut off nav lights/AIS/VHF)",
        properties: {
          urgency: {
            type: "object",
            title: "Urgency Tuning",
            description:
              "Override the urgency calculation knobs. Defaults are sensible for most vessels; leave a field blank to keep its default",
            properties: {
              weights: {
                type: "object",
                title: "Score Weights",
              ...
              thresholds: {
                ...
                description: "Score boundaries mapping to each urgency level",
                properties: {
                  normal: {
                    type: "number",
                    default: 0.1,
                  ...
                  low: { type: "number", default: 0.5, minimum: 0, maximum: 1 },
                  medium: {
                    type: "number",
                    default: 0.7,
                    minimum: 0,
              ...
              batterySoCTiers: {
                type: "object",
                title: "Battery SoC Tiers",
                description:
                  "SoC fractions (0-1) at which each urgency level kicks in for engine/battery advisories",
                properties: {
                  alarm: {
                    type: "number",
                    default: 0.1,
              ...
              minGoodOutputHoursByType: {
                type: "object",
                title: "Per-Deployable Reluctance (minimum good-output hours)",
                description:
                  "Default minimum good-output hours to justify deploying each deployable type — the reluctance gate. A short forecast window caps a deploy suggestion at Low (visual, no sound). Hydro is Low (1h), FLINsail is Medium (2h), wind generators are High (8h). A per-device reluctance pulldown overrides these.",
                properties: {
                  hydro: {
                    type: "number",
                    default: 1,
