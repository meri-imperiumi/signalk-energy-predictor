...
const { sunPosition, maxIrradiance, irradianceFromCloudCover, oktasToFraction } = require("./solar.js");
 ...
 * @typedef {{time: Date, ghi: number, cloudCover: number|null, gustSpeedKnots: number|null}} ForecastPoint
 */
...
async function fetchOpenMeteo(latitude, longitude) {
  ...
  url.searchParams.set("hourly", "shortwave_radiation,wind_gusts_10m");
  ...
  try {
    ...
    return data.hourly.time.map((time, i) => ({
      ...
      cloudCover: null,
      gustSpeedKnots: data.hourly.wind_gusts_10m?.[i] ?? null,
    }));
  } finally {
...
class IngestionFSM {
  ...
  /**
   * Checks network status for fallback decision.
   *
   * @returns {Promise<NetworkStatus>}
   */
  async getNetworkStatus() {
    const status = this.app.getSelfPath("network.wan.status");
    return { wanOnline: status === "online" };
  }
  ...
  async fetchFromTier(tier) {
    ...
    switch (tier) {
      ...
      case Tier.CLEAR_SKY: {
        return generateClearSkyForecast(new Date(), FORECAST_HOURS, latitude, longitude);
      }
